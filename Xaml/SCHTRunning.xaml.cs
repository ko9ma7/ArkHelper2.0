...
namespace ArkHelper.Pages.OtherList
{
    ...
    public partial class SCHTRunning : Page
    {
                ...
                void MainTask(Data.SCHT.SCHTData schtData)
                {
                    Thread thread = new Thread(() =>
                    {
                        ...
                        void InitData()
                        {
                            ...
                            freshTime = ArkHelperDataStandard.GetGameFreshTime(schtData.server.id);

                            if (freshTime < DateTime.Now.Hour
                            && freshTime + 12 > DateTime.Now.Hour)
                            { AMmode = true; }
                            else { AMmode = false; }
                            if (schtData.server.id == "EN")
                                week = DateTime.Now.AddHours(-15).DayOfWeek;
                        ...
                        void ListenFresh()
                        {
                            var task = new Task(() =>
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    {
                                        Info("处于游戏刷新时间，正在自动退出", Output.InfoKind.Warning);
                                        ExitGame();
                                        ThisTaskStopEvent?.Invoke(this, true);
                                        break;
                                    } 
                                }
                            });
                        ...
                        UnitInited:;
                            MBCore MB = new MBCore(mode: MBCore.ModeType.san, allowToRecoverSantiy: false);
                            MB.MessageUpdated += Info;
                            MB.Run();
        ...
        static EventHandler<bool> AllStopEvent;
        static EventHandler<bool> ThisTaskStopEvent;

        private void StopClick(object sender, RoutedEventArgs e)
        {
            AllStopEvent(sender,true);
        }
